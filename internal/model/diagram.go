...
func Mermaid(covered Coverage) string {
	...
	for _, e := range Edges {
		...
		case !published:
			...
			arrow = "-.->"
			label = e.Label + "<br/><i>not published upstream</i>"
...
func ASCII(covered Coverage) string {
	...
	b.WriteString("                           ▼\n")
	b.WriteString("                  ┌──────────────────┐\n")
	b.WriteString("                  │    Supervisor    │  version embeds the vCenter\n")
	b.WriteString("                  │ v1.32.9+vmware.2 │  line: ...-fips-vsc9.1.0.0200\n")
	b.WriteString("                  └────────┬─────────┘\n")
	b.WriteString("                           │ runs\n")
	b.WriteString("                           ▼\n")
	b.WriteString("                  ┌──────────────────┐\n")
...
func Explanation(covered Coverage) string {
	...
	for _, e := range Edges {
		...
		marker := ""
		if !covered(from.ID, to.ID) {
			marker = " *(not published upstream)*"
		}
		fmt.Fprintf(&b, "- **%s → %s** — %s%s\n", from.Label, to.Label, e.Prose, marker)
	}
	return b.String()
}
...
func Doc(covered Coverage) string {
	...
	b.WriteString("Supervisor it manages; the guest clusters move last.\n\n")
	b.WriteString("## A note on the data\n\n")
	b.WriteString("Not every pair above is published upstream. Where a pair is marked\n")
	b.WriteString("*not published*, this tool infers the relationship through the products that\n")
	b.WriteString("are published — and always labels the result inferred rather than verified.\n")
